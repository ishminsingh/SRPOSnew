...
import static dev.ishmin.srpos.Billing.total;
        ...
        textView.setText(Float.toString(BillingFragment.total));
            ...
            try {
                ...
                if (cno.getText().toString().length() == 10 && discount.getText().toString().length() > 0)
                {
                    try
                    {
                        MainActivity.SRPOS.execSQL("INSERT INTO Sales(customerno,date,billamount,discount,status)VALUES('" + Integer.parseInt(cno.getText().toString()) + "','" + date + "','" + total + "'," + Float.parseFloat(discount.getText().toString()) + ",'" + status + "')");
                    } catch (Exception e)
                    {
                        e.printStackTrace();
                    }
